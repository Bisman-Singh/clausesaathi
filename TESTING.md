...
| Analysis              | wire-to-strict conversion, bounds, one bad item dropped not the brief, deadline shapes, citation dropping, statute attachment by severity, result cache, full pipeline through the AI SDK's mock model            |
| Deadlines             | UTC date arithmetic, anchor mapping, resolution, ordering                                                                                                                                                         |
| Compare               | similarity, word diff, alignment by content or shared title, one-to-one matching, long-clause fallback, punctuation-only edits, explanation cap                                                                   |
...
| Components            | every component and page, with axe on the form, results, diff, chrome and content pages; keyboard interaction through Testing Library user-event; Hindi names and dates; statute sources in Q&A; drop zone states |
